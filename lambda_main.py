...
def wait_for_processed_image(processed_bucket, processed_key, timeout=60, interval=5):
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            print(f"Processed image {processed_key} not yet available, waiting...")
            time.sleep(interval)
...
def main():
    ...
    if object_key:
        # Construct the processed key (if your Lambda function modifies the key)
        ...
        if wait_for_processed_image(processed_bucket, processed_key, timeout=60, interval=5):
